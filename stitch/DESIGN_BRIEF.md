...
- FixLoop Smoke Board turns the user's request into a directly usable web workflow. The first experience must be the actual web product behavior, not a marketing landing page or placeholder demo.
- - FR-001: Build a tiny single-page app called FixLoop Smoke Board. Keep it simple. It should show three compact status cards, one refresh button that updates a visible timestamp, and one Ready/Paused toggle. Use the existing stack and do not add navigation, auth, analytics, settings, or unrelated modules.
...
- Status Utility - FixLoop Smoke Board

## SCREEN_SPECS
SCREEN_SPEC_1:
- exact_screen_title: Status Utility - FixLoop Smoke Board
